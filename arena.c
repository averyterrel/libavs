...
arena_t * arena_create(size_t max) {
  ...
  if (!arena) {
    avs_debug("Arena: creation failure\n");
    return NULL;
  }
  arena->max = max;
  return arena;
...
arena_t * arena_resize(arena_t * arena, size_t max) {
  if (!arena) {
    avs_debug("Arena: null arena\n");
    return NULL;
  }
  if ((arena->head - arena->base) > max) {
    avs_debug("Arena: tried to resize below size\n");
    return NULL;
  }
  arena->max = max;
  return arena;
...
uintptr_t arena_malloc(arena_t * arena, size_t size) {
  ...
  if (newsize > arena->max) {
    avs_debug("Arena: grew too large\n");
    return (uintptr_t)NULL;
  }

  uintptr_t new_base = (uintptr_t)realloc((void*)arena->base, newsize);
  if (!new_base) {
    avs_debug("Arena: realloc failure\n");
    return (uintptr_t)NULL;
  }
  arena->base = new_base;
